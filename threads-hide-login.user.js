...
(function () {
  ...
  //
  // 多數情況下這裡不會有實際效果：overlayTargetFor 已經把整個 portal
  // 隱藏，遮罩身為它的後代自然跟著消失（實測確認過，此時全頁掃描為 0 筆）。
  // 保留它是為了登入牆與媒體播放器共用同一個 portal 的情況 —— 那時
  // overlayTargetFor 會被播放器擋下而停在登入分支，與該分支平行的遮罩
  // 就不在隱藏範圍內，只有這裡會處理到。
  //
  // 換句話說，看起來像 dead code，但砍掉會在共用 portal 時留下暗幕。
  //
